...
ビュー (View): v_cross_border_financial_summary を作成し、売上高の総額、履歴レート、実質入金売上純額、および手数料コスト損益を完全にカプセル化。クエリ実行時の重複計算を不要化。
...
3つのテーブル間のリレーションシップ（ER図）: MARKETS（市場マスタ）、MONTHLY_SALES（月別売上テーブル）、EXCHANGE_RATES（履歴レートテーブル）間の 1 : N 主・外部キーリレーションを明確に定義。
...
積み上げ棒グラフ: フロントエンドインターフェースの棒グラフを上下の積み上げ構造に最適化。下部主体に実質純額と現地元通貨（TWD）を表現し、頂部には高い識別性を持つカラーブロックで「手数料コスト損益のスタック」を重ねて表示。
